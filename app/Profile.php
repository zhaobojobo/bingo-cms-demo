...
class Profile
{
    ...
    private function parallel($data)
    {
        ...
        foreach ($data as $langId => $_data) {
            ...
            if ($langId == 'all') {
                $lang = '';
            } elseif (array_key_exists($langId, $this->languages)) {
                $lang = $langId;
            }
            if (! is_null($lang)) {
     ...
     * @return array
     */
    public function findRows($id, $model_id = 0, $lang = null)
    {
        $rows = [];
        foreach ($this->languages as $langId => $language) {
            $all           = $this->findLang($id, '', $model_id);
            $rows[$langId] = array_merge($all, $this->findLang($id, $langId, $model_id));
        }
    ...
    public function fields($model_id = 0)
    {
        $fields = [];
        if ($model_id) {
            $models = (new \Site\Models\Model())->children($model_id);
