...
if (!$i18n): ?>
    <div class="form-group">
        ...
        <input type="password" name="<?= $key ?>" id="<?= $key ?>" class="form-control input_normal"
               value="<?= $setting['value'] ?>">
    </div>
<?php
else: ?>
        ...
        <div class="tab-content setting-tab-content" id="nav-tabContent-<?= $key ?>">
            ...
            foreach ($languages as $language => $name): ?>
                ...
                if ($language_default == $language): ?>show active<?php
                    ...
                    <input type="password" name="<?= $key ?>[<?= $language ?>]" id="<?= $key ?>"
                           class="form-control input_normal"
                           value="<?= $setting['value'] ? $setting['value'][$language] : '' ?>">
                </div>
            <?php
            endforeach; ?>
        </div>
