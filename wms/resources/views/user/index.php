                    ...
                    <table class="table table-white table-bordered bingo_table">
                        ...
                        foreach ($data['pageData']['rows'] as $row) : ?>
                            <tr>
                                ...
                                <td><?= $row['email'] ?></td>
                                <td><?php
                                    if ($row['role']): ?><?= $row['role']->getName() ?><?php
                                    endif ?>
                                </td>
                                <td>
                                    <a class="button-gray bingo_button icon_button" title="<?= Helper::_('Edit') ?>"
                                       href="<?= Helper::getUrl('/user/edit/' . $row['id']) ?>">
                                        <i class="fa fa-pencil" aria-hidden="true"></i>
                                    </a>
                                    <a class="button-gray bingo_button icon_button"
                                       title="<?= Helper::_('Chang Password') ?>"
                                       href="<?= Helper::getUrl('/user/password/' . $row['id']) ?>">
                                        <i class="fa fa-key" aria-hidden="true"></i>
                                    </a>
                                    <a class="button-gray bingo_button icon_button delete"
                                       title="<?= Helper::_('Delete') ?>" href="<?= Helper::getUrl('/user/delete') ?>"
                                       data-id="<?= $row['id'] ?>">
