...
<textarea name="content" id="content-<?= $data['langId'] ?>"
          class="editor-<?= $data['langId'] ?>"><?= $data['content'] ?></textarea>
<script>
    var editors = editors || {};
    (function () {
        var editorEl = ".editor-<?= $data['langId'] ?>";
        var editorName = "<?= $data['langId'] ?>";
        editors[editorName] = new FroalaEditor(editorEl, {
            theme: 'dark', language: '<?= $data['langId'] ?>',
            fontFamily: {
        ...
        setTimeout(function () {
            var btn = $('<div class="fr-btn-grp fr-float-left"></div>')
                .append($(editorEl).parents('.edit-section').find('[data-target="#content-templates-modal"]'));

            $('.fr-toolbar').find('.fr-btn-grp.fr-float-left').last().after(btn);
