...
class FormField extends Base
{
    ...
    public static function rendList($list, $fid)
    {
        ...
        foreach ($list as $item) :
            ...
            $html           .= "<div class=\"col-2\"><span>{$item->name}</span><button class=\"d-none\" type=\"button\" data-toggle=\"modal\" data-target=\"#edit-title-modal\" data-id=\"{$item->id}\" data-lang=\"{LANG}\"></button></div>";
            $html           .= "<div class=\"col-2\">{$input}</div>";
            $html           .= "<div class=\"col-3\">{$item->__data[DEFAULT_LANG]->label}</div>";
     ...
     * @return string
     */
    public static function html($field, $langId)
    {
        $html        = '';
        $name        = $field['name'];
        $id          = $field['name'] . '-' . $langId;
        $label       = $field['__data'][$langId]['label'];
        $placeholder = $field['__data'][$langId]['placeholder'];
        $tip         = '';
        if ($field['__data'][$langId]['tip']) {
            $tip = "\n" . '<small id="emailHelp" class="form-text text-muted">'
                . $field['__data'][$langId]['tip'] . '</small>';
        }
